...
l=L;
v = 16.0;                      % Constant forward speed (m/s)
dt = 0.05;                    % Simulation time step (seconds)
lookahead = 2.0;              % Lookahead distance for pure pursuit (meters)
swath_width =5;              % Width between swaths (meters)
num_swaths = 9;               % Input Number of swaths to cover here
field_length = 40;            % Length of the field (meters)
turn_radius = swath_width / 2; % Radius of U-turn arcs (meters)

%% Generate complete path (straight swaths + outward U-turns on correct side)
path = [];
for i = 1:num_swaths
...
%% Simulation loop
  count =1;
figure('Name','Tractor Swath Tracking with Proper Outward U-Turns');
while true
    if idx >= size(path, 2)
    ...
    % Tractor model kinematics update
    x = x + v*cos(theta)*dt;
    y = y + v*sin(theta)*dt;
...
% %% Animation
% veh_box = 2*[-b,b,b,l,l,b,b,-b,-b;-D,-D,-d,-d,d,d,D,D,-D];
